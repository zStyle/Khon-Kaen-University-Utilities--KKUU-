...
public class AlarmService_Service extends Service {
	...
	public void onCreate() {
		...
		showNotification();
		// find the next waketime
		Date today = new Date(java.lang.System.currentTimeMillis());
		Date wakeday = getWakeDay(today);
		int wakeDayInt = wakeday.getDay();
		String wakeTimeString = setAlarm.wakeTimeWeek[wakeDayInt];

		int firstClassHourWakeday = Integer.parseInt(wakeTimeString.substring(
				0, 2));
		int firstClassMinWakeday = Integer.parseInt(wakeTimeString.substring(3,
				5));
		wakeday.setHours(firstClassHourWakeday);
		wakeday.setMinutes(firstClassMinWakeday);
		wakeday.setTime(wakeday.getTime() - (60000 * setAlarm.beforeTime));

		// set the next wakeup time
		Intent intent = new Intent(AlarmService_Service.this,
				AlarmService_Service.class);
		PendingIntent sender = PendingIntent.getBroadcast(
				AlarmService_Service.this, 0, intent, 0);
		AlarmManager am = (AlarmManager) getSystemService(ALARM_SERVICE);
		am.set(AlarmManager.RTC_WAKEUP, wakeday.getTime(), sender);
		Toast.makeText(getApplicationContext(),
				"Next waketime is" + wakeday.toLocaleString(),
				Toast.LENGTH_SHORT).show();
		nextTime = "Next waketime is" + wakeday.toLocaleString();
	...
	private void showNotification() {
		...
		PendingIntent contentIntent = PendingIntent.getActivity(this, 0,
				new Intent(this, setAlarm.class), 0);

		// Set the info for the views that show in the notification panel.
		notification.setLatestEventInfo(this,
				getText(R.string.alarm_service_label), nextTime, contentIntent);
